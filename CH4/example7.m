...
% simulates a battery pack built from SCMs. The pack comprises Ns cells
% wired in series to make SCMs; it then has Np SCMs wired in series to
...
outFile = 'ROM/ROMoutSCM.mat'; % Where to save the simulation results
architecture = 'SCM';          % The pack architecture

Ns = 3; % Number of cells wired in series in every SCM
Np = 3; % Number of SCMs wired in parallel

% The pack simulation is slow since optimizations must be done every time
% step to determine the current split between SCMs. When the simulation is
% performed, we save the results in an output file. If this file exists,
% don't re-run the simulation but instead use the precomputed results. If
% you want to run the simulation, then delete (or rename) the output file.
...
%% Plot results
close all
load('ROM/ROMoutSCM.mat','ROMout');  % Load the precomputed simulation data
warning('on','MATLAB:dispatcher:UnresolvedFunctionHandle');

[Ns,Np] = size(ROMout); % Battery-pack size
t = ROMout(1,1).time;   % Time vector for plotting

% Plot voltages of all SCMs
for kp=1:Np
  figure(kp)
  for ks=1:Ns
    plot(t/60,ROMout(ks,kp).Vcell); hold on
  end
  title(sprintf('All cell voltages for SCM %d',kp));
...
title('All cell voltages for all SCMs');
xlabel('Time (min)'); ylabel('Voltage (V)'); grid on

% Plot currents of all SCMs
for kp=1:Np
  figure(Np+1+kp)
  for ks=1:Ns
    plot(t/60,ROMout(ks,kp).Iapp); hold on
  end
  title(sprintf('All cell currents for SCM %d',kp));
  xlabel('Time (min)'); ylabel('Current (A)'); grid on
end

% Plot currents for all cells in all SCMs
figure(2*Np+2)
for ks=1:Ns
  for kp=1:Np
    plot(t/60,ROMout(ks,kp).Iapp); hold on
...
title('All cell currents in all SCMs');
xlabel('Time (min)'); ylabel('Current (A)'); grid on
...
title('Side-reaction overpotential for all cells in all SCMs');
