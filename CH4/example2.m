% This example reproduces the results in section 4.7 of the book. It
% simulates a single charge-neutral UDDS profile around a 60% SOC setpoint
% and 25 degC with NO blending for the ROM.
% 
...
inputFile = 'inputUDDS.xlsx';  % profile of current/temp vs time
...
ROMoutFile = 'ROM/OUT_NMC30_UDDS_NONB.mat'; % ROM simulated output
FOMfile    = 'FOM/FOM_NMC30_UDDS.mph'; % generated FOM
FOMoutFile = 'FOM/OUT_NMC30_UDDS.mat'; % FOM simulated output
...
if ~exist(ROMfile,'file') % then, need to create the ROM
  ...
  % ROM = genROM(cellData,xraData,'HRA_OLD'); % The old broken HRA
  ROM = genROM(cellData,xraData,'HRA');     % WH new corrected HRA
...
ROMout = simROM(ROM,simData,'nonBlend'); 
% ROMout = simROM(ROM,simData,'outBlend'); 
save(ROMoutFile,'ROMout');
