...
% Specifically, it simulates the loss of active material in the negative
% electrode, at 0% SOC.
% 
...
qEps = 0.0015; % fractional material loss per iteration
...
% Arrow for theta100n
offset = -0.03; len = 0.08;
X0 = theta(2,1); Y0 = vHist100(1);                       [X0f,Y0f] = ds2nfu(X0,Y0);
XM = theta(2,round(ind/6)); YM = vHist100(round(ind/6)); [XMf,YMf] = ds2nfu(XM,YM);
XN = theta(2,round(ind/3)); YN = vHist100(round(ind/3)); [XNf,YNf] = ds2nfu(XN,YN);
...
XM = theta(4,round(0.65*ind)); YM = vHist100(round(0.65*ind));    [XMf,YMf] = ds2nfu(XM,YM);
